...
public class CardSoln3 {
	...
	System.out.println(card);
        ...
        PokerHand idk=new PokerHand();
        PokerHand hand=idk.Deal();
        System.out.printf("idk instance of PokerHand: %s \nidk instance of Deck: %s", idk instanceof PokerHand, idk instanceof Deck);
        hand.print();
	...
	// make a subdeck
	Deck subdeck=deck.subdeck(8, 12);
	System.out.printf("hand Deck: ");
        subdeck.print();
...
class PokerHand extends Deck {
     ...
     */
    public PokerHand() {
	this.cards2=new Card [52];
	int index=0;
	for (int suit=0; suit<=3; suit++) {
	    for (int rank=1; rank<=13; rank++) {
		this.cards2[index]=new Card(suit, rank);
		index++;
 	    }
	}
    }
    public PokerHand subpokerhand(int low, int high) {
	PokerHand sub=new PokerHand(high-low+1);
	for (int i=0; i<sub.cards2.length; i++) {
	    sub.cards2[i]=cards2[low+i];
	}
	return sub;
    }
    public void print() {
        for (Card card : cards2) {
            //System.out.printf("Card deck: ");
            System.out.printf("\n%s",cards2);
            //card.print();
        }
    }
    public PokerHand Deal() {
        PokerHand deck=new PokerHand(52);
        deck.shuffle();
        PokerHand hand=(PokerHand)deck.subpokerhand(0, 4);
        return hand;
    }
}
